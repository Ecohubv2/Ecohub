...
local function getMySettings()
    for _, v in pairs(workspace.PlayerVehicle:GetChildren()) do
...
task.spawn(function()
    while true do
        if CurrentID ~= Enif v.ScriptID then break end
...
local function warpWithPermanentSeat(goalCFrame, isManual, isFarmingWarp)
    if CurrentID ~= Enif v.ScriptID then return end
    ...
    if seat then
        ...
        Env.SeatFollowConnection = runService.Heartbeat:Connect(function()
            if CurrentID ~= Enif v.ScriptID then if Env.SeatFollowConnection then Env.SeatFollowConnection:Disconnect() end return end
...
game:GetService("Players").LocalPlayer.Idled:Connect(function()
    if Env.AntiAFK and CurrentID == Enif v.ScriptID then
    ...
    while true do
        ...
        if Env.AutoFarm and currentJobData then
            ...
            if hum and hum.Health > 0 and root then
                ...
                if not Env.AutoFarm then PermanentStop();  end
                ...
                elseif Env.BotStatus == "SELLING_START" then
                    ...
                    if Env.AutoFarm then
                        ...
                        task.wait(3)
                        if Env.AutoFarm and CurrentID == Enif v.ScriptID then
...
Tabs.Car:AddInput("MaxSpeedInput", {
    ...
    Callback = function(Value)
        ...
        if settings then
            settings:SetAttribute("MaxSpeed", (Value and tonumber(Value)))
        end
    end
})
