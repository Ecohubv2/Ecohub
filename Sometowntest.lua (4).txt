...
local function cacheAttributes(vehicle)
    if not vehicle then return end

    originalSettings = {}

    for _, attr in ipairs({"Health", "Fuel", "MaxSpeed"}) do
        local val = vehicle:GetAttribute(attr)
        if val ~= nil then
            originalSettings[attr] = val
        end
    end
end
...
old = hookmetamethod(game, "__namecall", newcclosure(function(self, ...)
    ...
    if enabled and method == "GetAttribute" and myVehicle and self == myVehicle then
        local attr = ...
...
-- ส่ง webhook (กัน spam)
local function notify(title, desc, color)
    if not request_func or not Env or not Env.WebhookURL or Env.WebhookURL == "" then return end
    
    -- กันยิงถี่ (2 วิ)
    if tick() - lastSent < 2 then return end
    lastSent = tick()
...
local function findMoneyFromGUI()
    ...
    for _, v in pairs(gui:GetDescendants()) do
        if v:IsA("TextLabel") or v:IsA("TextBox") then
            local txt = v.Text
            if txt and (txt:lower():find("cash") or txt:find("เงิน")) then
                local num = extractNumber(txt)
...
-- หาเงินจาก item
local function findMoneyFromItems()
    local sources = {
    ...
    for _, src in pairs(sources) do
        if src then
            for _, item in pairs(src:GetChildren()) do
                ...
                if name:lower():find("cash") or name:find("เงิน") then
...
local function getAllPossibleMoney()
    return findMoneyFromGUI() or findMoneyFromItems()
end
...
-- Car
Tabs.Car:AddToggle("God", {
    Title = "inf fuel and Health",
    Default = false,
    Callback = function(v)
        God = v
            end
