...
local function getMySettings()
    local vehicles = workspace:FindFirstChild("PlayerVehicle")
    if not vehicles then return nil end

    for _, v in pairs(vehicles:GetChildren()) do
        if v:GetAttribute("Owner") == player.Name then
            return v:FindFirstChild("Settings", true)
        end
    end
end
...
local function notify(title, desc, color)
    if not request_func or not Env or Env.WebhookURL == "" then return end
    ...
    for _, src in pairs(sources) do
        if src then
            for _, item in pairs(src:GetChildren()) do
                if item.Name:find("เงิน") or item.Name:lower():find("cash") then
...
task.spawn(function()
    while task.wait(1) do
        if not Env or CurrentID ~= Env.ScriptID then break end
...
task.spawn(function()
    ...
    while true do
        ...
        -- ❌ แทน continue
        if not (Env.AutoFarm and currentJobData) then
            PermanentStop()
        else
            local char = Player.Character
            ...
            if char then
                ...
                if hum and root and hum.Health > 0 then
                    ...
                    if Env.BotStatus == "CHECKING" then
                        ...
                        pcall(function()
                            ...
                            for _, item in pairs(body:GetChildren()) do
                                if item.Name == currentJobData.Item then
                                    local amt = item.Main.Amount.Text:match("^(%d+)")
                                    if amt and tonumber(amt) >= currentJobData.Max then
                                        alreadyFull = true
                                        break
                                    end
                                end
                        ...
                        if alreadyFull then
                            Env.BotStatus = "SELLING_START"
                        else
                            warpWithPermanentSeat(currentJobData.Pos, false, true)
                            Env.BotStatus = "FARMING"
                        end
                    end

                    -- ================= DISTANCE =================
                    local distance = (root.Position - currentJobData.Pos.Position).Magnitude
                    if Env.BotStatus == "FARMING" and distance > Max_Distance_From_Spawn then
                        warpWithPermanentSeat(currentJobData.Pos, false, true)
                    end
                    ...
                    -- ================= SELL =================
                    if Env.BotStatus == "SELLING_START" then
                        ...
                        if Env.AutoFarm and CurrentID == Env.ScriptID then
                            ...
                            task.spawn(function()
                                local t = tick() + Delay_WalkAtMarket
                                while tick() < t and Env.AutoFarm do
                            ...
                            until not isHolding or not Env.AutoFarm or CurrentID ~= Env.ScriptID
                    ...
                    -- ================= INIT =================
                    if Env.BotStatus == "INIT" then
                        warpWithPermanentSeat(currentJobData.Pos, false, true)
                        Env.BotStatus = "FARMING"
                    end

                    -- ================= FARM =================
                    if Env.BotStatus == "FARMING" then
                        ...
                        if not (targetItem and targetItem.Parent and targetItem:FindFirstChild("TouchTransmitter")) then
                            ...
                            -- 🔥 CACHE ลดแลค
                            if not CachedItems[currentJobData.Item] then
                                CachedItems[currentJobData.Item] = {}
                                for _, v in pairs(workspace:GetDescendants()) do
                                    if v:IsA("BasePart") and v.Name == currentJobData.Item then
                                        if v:FindFirstChildOfClass("TouchTransmitter") then
                                            table.insert(CachedItems[currentJobData.Item], v)
                                        end
                                    end
                                end
                            end
                            ...
                            for _, v in pairs(CachedItems[currentJobData.Item]) do
                            ...
                            Env.CurrentFarmTargetItem = closest
                            targetItem = closest
                        end
                        ...
                        if targetItem then
                            ...
                            -- รี cache ถ้าหาไม่เจอ
                            CachedItems[currentJobData.Item] = nil
                        end
                    end
...
Window:SelectTab(1)
Library:Notify({ Title = "_kopeas", Content = "Loaded Fully", Duration = 5 })
