...
local function usePickaxeOnce()
    pcall(function()
        game:GetService("ReplicatedStorage")
            ...
            :FireServer("fire", nil, "Tools", "Pickaxe")
    end)
end
...
Tabs.Farm:AddToggle("AutoMine", {
    ...
    Callback = function(v)
        ...
        if v then
            task.spawn(function()
                while Env.AutoMine and CurrentID == Env.ScriptID do
                    task.wait(0.5)
                    ...
                    if cur < max then
                        ...
                        pcall(function()
                            game:GetService("ReplicatedStorage")
                                ...
                                :WaitForChild("NetworkFramework")
                                :WaitForChild("NetworkEvent")
                                :FireServer("fire", nil, "Process", "Rock")
                        end)
                        ...
                        local fragments = Player:GetAttribute("Rockfragments") or 0
                        ...
                        if fragments > 0 then
                            ...
                            game:GetService("ReplicatedStorage")
                                .Modules.NetworkFramework.NetworkEvent
                                :FireServer("fire", nil, "Economy", "Rockfragments", fragments)
...
local function buyBait()
    ...
    task.wait(2)
    Remote:FireServer("fire", nil, "Supermarket", "Bait", 150)
    task.wait(2)
end
...
Tabs.Farm:AddToggle("AutoFishAll", {
    Title = "Auto Fish",
    Default = false,
    Callback = function(v)
        Env.AutoFishAll = v
