...
-- removed duplicate multifarm loop

                if Env.MultiFarmLogic then
                    Env.MultiFarmLogic(count,currentJobData)
                end
            end
        end
    end
...
-- =====================================================
-- ECO HUB V3 ULTRA PATCH (Performance + Smart Systems)
-- =====================================================
...
function Env.GetNearestFarmItem(root)
    ...
    local parts = workspace:GetPartBoundsInRadius(root.Position,80)

    for _,v in ipairs(parts) do

        local tt = v:FindFirstChildOfClass("TouchTransmitter")
...
-- ==========================
-- SUPER WARP (fast & safe)
-- ==========================
function Env.SuperWarp(goal)
...
warn("ECO HUB V3 ULTRA PATCH LOADED")
