...
-- mine
local RockFarmEnabled = false
local firedThisLife = false

local rockPos = CFrame.new(-36.8056488, 32.9587746, -2485.83887, 0.852164865, -9.04716035e-09, 0.523273408, 3.88641164e-09, 1, 1.0960421e-08, -0.523273408, -7.30643013e-09, 0.852164865)

local function usePickaxeOnce()
    if firedThisLife then return end
    firedThisLife = true
    ...
    pcall(function()
        ...
        :FireServer("fire", nil, "Use Tool", "Pickaxe")
    end)
end

local function getRockAmount()
    local body = Player.PlayerGui:FindFirstChild("Inventory")
    body = body and body:FindFirstChild("CanvasGroup")
    body = body and body:FindFirstChild("Main")
    body = body and body:FindFirstChild("Body")

    if not body then return 0, 100 end

    local rock = body:FindFirstChild("Rock")
    if not rock then return 0, 100 end

    local amount = rock:FindFirstChild("Main") and rock.Main:FindFirstChild("Amount")
    if not amount then return 0, 100 end
...
local function StartRockFarm()
    task.spawn(function()
        while RockFarmEnabled do
            task.wait(1)

            local current, max = getRockAmount()
...
-- Farm rock
Tabs.Farm:AddToggle("AutoRockFull", {
    Title = "Auto Mine",
    Default = false,
    Callback = function(v)
        RockFarmEnabled = v
